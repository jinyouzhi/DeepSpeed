...
__global__ void attn_softmax_v2(__nv_bfloat16* vals,
                                __nv_bfloat16* mask,
                                __nv_bfloat16* alibi,
                                float layer_scale,
                                bool triangular,
                                ...
                                int reduceWidth)
{
    ...
    const __nv_bfloat16 zero_h = __float2bfloat16(0.f);
    ...
    if (iter_offset < total_count) {
            ...
            if ((!triangular || ((data_id >> 2) <= seq_id4)) && (data_id >> 2) >= window_stride4 &&
                ...
                if ((sequence_length - data_id) >= 4) {
                    low_data[i].x = data_id > window_stride
                                        ? __bfloat162float(vals[data_id]) * layer_scale
                                        : minus_infinity;
                    low_data[i].y = ((!triangular || ((data_id + 1) <= seq_id)) &&
                                     (data_id + 1) > window_stride)
                                        ? __bfloat162float(vals[data_id + 1]) * layer_scale
                                        : minus_infinity;
                    high_data[i].x = ((!triangular || ((data_id + 2) <= seq_id)) &&
                                      (data_id + 2) > window_stride)
                                         ? __bfloat162float(vals[data_id + 2]) * layer_scale
                                         : minus_infinity;
                    high_data[i].y = ((!triangular || ((data_id + 3) <= seq_id)) &&
                                      (data_id + 3) > window_stride)
                                         ? __bfloat162float(vals[data_id + 3]) * layer_scale
                                         : minus_infinity;
                    if (alibi) {
                        low_data[i].x =
                            low_data[i].x + __bfloat162float(alibi[data_id + alibi_offset]);
                        low_data[i].y =
                            low_data[i].y + __bfloat162float(alibi[data_id + alibi_offset + 1]);
                        high_data[i].x =
                            high_data[i].x + __bfloat162float(alibi[data_id + alibi_offset + 2]);
                        high_data[i].y =
                            high_data[i].y + __bfloat162float(alibi[data_id + alibi_offset + 3]);
                    }
                    if (mask) {
                        low_data[i].x += __bfloat162float(mask[data_id + mask_offset]);
                        low_data[i].y += __bfloat162float(mask[data_id + mask_offset + 1]);
                        high_data[i].x += __bfloat162float(mask[data_id + mask_offset + 2]);
                        high_data[i].y += __bfloat162float(mask[data_id + mask_offset + 3]);
                    }
                } else {
                    low_data[i].x = data_id > window_stride
                                        ? __bfloat162float(vals[data_id]) * layer_scale
                    ...
                    if (alibi) {
                        ...
                        if ((data_id + 2) < sequence_length)
                            high_data[i].x = high_data[i].x + high_data[i].x +
                                             __bfloat162float(alibi[data_id + alibi_offset + 2]);
                    }
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (data_id < sequence_length) {
                if ((sequence_length - data_id) >= 4) {
                    vals[data_id] = __float2bfloat16(low_data[i].x / sum);
                    vals[data_id + 1] = __float2bfloat16(low_data[i].y / sum);
                    vals[data_id + 2] = __float2bfloat16(high_data[i].x / sum);
                    vals[data_id + 3] = __float2bfloat16(high_data[i].y / sum);
                } else {
                    vals[data_id] = __float2bfloat16(low_data[i].x / sum);
                    if ((data_id + 1) < sequence_length)
                        vals[data_id + 1] = __float2bfloat16(low_data[i].y / sum);
