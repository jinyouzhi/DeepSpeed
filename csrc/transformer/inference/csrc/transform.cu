...
__global__ void bias_add_transform_0213(__nv_bfloat16* output,  // q
                                        __nv_bfloat16* k_cache,
                                        __nv_bfloat16* v_cache,
                                        const __nv_bfloat16* vals,  // qkv
                                        ...
                                        int max_out_tokens)
{
    ...
    __nv_bfloat162* vals_half = reinterpret_cast<__nv_bfloat162*>(&vals_arr);
    __nv_bfloat162* output_half = reinterpret_cast<__nv_bfloat162*>(&output_arr);
    ...
    if (cnt < 2 && rotary_dim > 0 && d3 < rotary_dim) {
        float4 q = vals_vec[d3];
        __nv_bfloat162* q_h = reinterpret_cast<__nv_bfloat162*>(&q);
        if (rotate_every_two) {
#pragma unroll
            for (int o = 0; o < 4; o++) {
                ...
                inv_freq = 1.0 / powf(10000.0, inv_freq) * (float)seq_id;
                float q_data[2];
                q_data[0] = __bfloat162float(q_h[o].x);
                q_data[1] = __bfloat162float(q_h[o].y);
                q_h[o].x = __float2bfloat16(-1.0 * q_data[1] * sinf(inv_freq) + q_data[0] * cosf(inv_freq));
                q_h[o].y = __float2bfloat16(q_data[0] * sinf(inv_freq) + q_data[1] * cosf(inv_freq));
            }
        }
        output_vec[d3] = q;
    } else
...
template <>
void launch_bias_add_transform_0213<__nv_bfloat16>(__nv_bfloat16* output,
                                                   __nv_bfloat16* k_cache,
                                                   __nv_bfloat16* v_cache,
...
__global__ void pad_add_transform_0213(__nv_bfloat16* output,
                                       const __nv_bfloat16* vals,
                                       int hidden_dim,
                                       int seq_length,
                                       int padded_seq_len,
...
template <>
void launch_pad_add_transform_0213<__nv_bfloat16>(__nv_bfloat16* output,
                                                  const __nv_bfloat16* vals,
                                                  int batch_size,
                                                  int hidden_dim,
                                                  int seq_length,
...
template <>
__global__ void bias_add_transform_0213<__nv_bfloat16>(__nv_bfloat16* output,
                                                       const __nv_bfloat16* vals,
                                                       const __nv_bfloat16* bias,
                                                       int hidden_dim,
                                                       int seq_length,
                                                       ...
                                                       int head_ext)
{
    ...
    float4 output_arr;
    __nv_bfloat162* vals_half = reinterpret_cast<__nv_bfloat162*>(&vals_arr);
    __nv_bfloat162* bias_half = reinterpret_cast<__nv_bfloat162*>(&bias_arr);
    __nv_bfloat162* output_half = reinterpret_cast<__nv_bfloat162*>(&output_arr);
...
__global__ void bias_add_transform_0213_v2(__nv_bfloat16* output,
                                           const __nv_bfloat16* vals,
                                           const __nv_bfloat16* bias,
                                           int hidden_dim,
                                           int seq_length,
...
template <>
__global__ void transform4d_0213<__nv_bfloat16>(__nv_bfloat16* out,
...
__global__ void transform4d_0213_v2(__nv_bfloat16* out,
                                    const __nv_bfloat16* in,
                                    int heads,
                                    int seq_length,
                                    int hidden_dim)
...
template <>
void launch_transform4d_0213<__nv_bfloat16>(__nv_bfloat16* out,
                                            const __nv_bfloat16* in,
                                            int batch_size,
                                            int heads,
                                            int seq_length,
                                            ...
                                            int trans_count)
{
    ...
    transform4d_0213<__nv_bfloat16>
