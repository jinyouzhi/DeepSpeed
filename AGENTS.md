...
- All non-merge commits MUST have a `Signed-off-by` line (use `--signoff`). Merge commits with `parent_count > 1` are exempt. Get the name and email from `git config user.name` / `git config user.email`.
